# python3.8
"""Contains some miscellaneous functions for EG3D training.

This file is mostly borrowed from:

https://github.com/NVlabs/eg3d/blob/main/eg3d/torch_utils/misc.py
"""
...
def assert_shape(tensor, ref_shape):
    if tensor.ndim != len(ref_shape):
        raise AssertionError(
            f'Wrong number of dimensions: got {tensor.ndim}, '
    ...
    for idx, (size, ref_size) in enumerate(zip(tensor.shape, ref_shape)):
        if ref_size is None:
            ...
            raise AssertionError(
                f'Wrong size for dimension {idx}: got {size}, '
                f'expected {ref_size}'
            )
