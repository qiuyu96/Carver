...
__all__ = ['EpiGRAFDataset']


class EpiGRAFDataset(BaseDataset):
    """Defines the dataset class for EpiGRAF training.
    ...
    def __init__(self,
                 ...
                 num_classes=None,
                 use_pose=True,
                 pose_meta='ffhq_512_posed/dataset.json'):
        ...
        self.dataset_name = pose_meta.split('/')[0]

    def _load_raw_poses(self, fp):
        poses = json.load(fp)['camera_angles']
        poses = dict(poses)
        poses = [
            poses[self.remove_root(fname, self.dataset_name).replace('\\', '/')]
    ...
    @staticmethod
    def remove_root(fname, root_name):
