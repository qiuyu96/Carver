...
from utils.parsing_utils import ListParamType
...
class BaseConfig(object):
    ...
    def get_options(cls):
        ...
        options['Working directory settings'].extend([
            ...
            cls.command_option(
                '--env_path', type=str, default='env.txt',
                help='To which to save Python environment list.'),
            cls.command_option(
            ...
            cls.command_option(
                ...
                help='Directory to save cached data, such as pre-trained '
                     'checkpoints, extracted features, etc. If not set, '
                     '`$HOME/cache/` will be used by default.'
            )
        ])
        ...
        options['Training dataset settings'].extend([
            cls.command_option(
                ...
                help='Path to the training dataset. NOTE: No tailing `/` is '
                     'accepted.'),
            cls.command_option(
        ...
        options['Validation dataset settings'].extend([
            cls.command_option(
                ...
                help='Path to the validation dataset. NOTE: No tailing `/` is '
                     'accepted.'),
            cls.command_option(
    ...
    def parse_options(self):
        ...
        self.config.env_filename = self.args.pop('env_path')
        ...
        self.config.cache_dir = self.args.pop('cache_dir')
        ...
        self.config.allow_fp16_reduced_precision_reduction = self.args.pop(
            'allow_fp16_reduced_precision_reduction')
