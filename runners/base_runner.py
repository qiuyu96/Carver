...
from utils.misc import set_cache_dir
...
class BaseRunner(object):
    ...
    @property
    def env_path(self):
        """Returns the path to the full python environment."""
        return os.path.join(self.work_dir, self.config.env_filename)
    ...
    def build_cache_dir(self):
        """Builds the cache directory."""
        if self.config.cache_dir:
            set_cache_dir(self.config.cache_dir)
        if self.is_chief:
    ...
    def log_basic_info(self):
        ...
        # Log docker image ID for reproducibility.
        # TODO: Find a better way to get Image ID.
        image_id = os.environ.get('DOCKER_IMAGE_ID', None)
        if image_id:
            self.logger.info(f'Docker: {image_id}\n')
        else:
            self.logger.info('Docker: Unknown\n')

        # Export python environment for reproducibility.
        executable = sys.executable
        version = os.popen(f'{executable} --version').readline().strip()
        ...
        env_info += ('\n' + '=' * 50 + '\n')
        self.logger.print(env_info)

        # Log PyTorch compiling environment for reproducibility.
        self.logger.info('PyTorch compiling environment:\n')
        compile_env_info = '=' * 50 + '\n'
        compile_env_info += f'Git version: {torch.version.git_version}\n'
        compile_env_info += f'Debug version: {torch.version.debug}\n'
        compile_env_info += f'CUDA version: {torch.version.cuda}\n'
        compile_env_info += f'cuDNN version: {torch.backends.cudnn.version()}\n'
        compile_env_info += f'NCCL version: {torch.cuda.nccl.version()}\n'
        compile_env_info += (torch.__config__.show() + '=' * 50 + '\n')
        self.logger.print(compile_env_info)
    ...
    def set_cudnn(self):
        ...
        torch.backends.cuda.matmul.allow_fp16_reduced_precision_reduction = (
            self.config.allow_fp16_reduced_precision_reduction)
    ...
    def build_train_loader(self):
        ...
        self.train_loader.dataset.save_items(self.data_dir, tag='train')
    ...
    def build_val_loader(self):
        ...
        self.val_loader = build_dataset(
            ...
            data_loader_kwargs=self.config.data.loader.copy())
        self.val_loader.dataset.save_items(self.data_dir, tag='val')
